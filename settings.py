# coding: utf-8
# Copyright (C) 2023 github.com/Killerzx/team
...
class proxy_setting():
    ...
    def clear_proxy():
        config.reset("Killerzx_proxies")
        Output("info").notime("Proxy Cache Cleared")
        sleep(1)   

    def add_proxies():
        path = utility.ask(f'Enter the path to the txt file containing proxies')
        folder_path = os.path.join(os.getenv('LOCALAPPDATA'), 'Killerzx_free')
        file_path = os.path.join(folder_path, 'Killerzx_proxies')
...
{Fore.LIGHTBLUE_EX}[09] LIGHT BLUE    {Fore.LIGHTGREEN_EX}[10] LIGHT GREEN {Fore.LIGHTCYAN_EX} [11] LIGHT CYAN   {Fore.LIGHTBLACK_EX}[12] LIGHT BLACK
    ...
    if selected_theme:
        config._set("Killerzx_theme", selected_theme)
...
def settings():
    ...
    if choice not in ["1", "2", "3", "4", "5"]:
        ...
        config._set("Killerzx_username", name)
        ...
        choice = utility.ask('Are you sure you want to exit Killerzx? (Y to confirm)')
        if choice.upper() == 'Y':
            utility.clear()
            os._exit(0)
        else:
